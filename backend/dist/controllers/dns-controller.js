...
exports.updateBlockedDomain = updateBlockedDomain;
exports.generateRpzZoneFile = generateRpzZoneFile;
exports.removeAllBlockedDomains = removeAllBlockedDomains;
exports.excludeDomain = excludeDomain;
exports.reincludeDomain = reincludeDomain;
exports.getExcludedDomains = getExcludedDomains;
exports.getAvailableBlocklists = getAvailableBlocklists;
exports.getMySubscriptions = getMySubscriptions;
exports.subscribeToBlocklist = subscribeToBlocklist;
exports.unsubscribeFromBlocklist = unsubscribeFromBlocklist;
exports.getExportFormats = getExportFormats;
exports.exportBlocklist = exportBlocklist;
exports.generateDownloadToken = generateDownloadToken;
exports.getDownloadLinkInfo = getDownloadLinkInfo;
exports.deleteDownloadToken = deleteDownloadToken;
exports.downloadBlocklistByToken = downloadBlocklistByToken;
...
async function getBlockedDomains(req, res) {
    try {
        ...
        const manualResult = await database_js_1.default.query(`SELECT id, domain, "blockedAt", NULL as source_list_id, NULL as source_list_name 
       FROM blocked_domains 
       WHERE tenant_id = $1`, [tenantId]);
        // 2. Fetch domains from subscribed blocklists (with subscription date for sorting)
        const subscribedResult = await database_js_1.default.query(`SELECT 
       ...
       WHERE s.tenant_id = $1`, [tenantId]);
        ...
        const exclusionsResult = await database_js_1.default.query(`SELECT domain FROM tenant_domain_exclusions WHERE tenant_id = $1`, [tenantId]);
...
async function addBlockedDomain(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
...
async function removeBlockedDomain(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
        ...
        const { id } = req.params;
        const result = await database_js_1.default.query('DELETE FROM blocked_domains WHERE id = $1 AND tenant_id = $2', [id, tenantId]);
...
async function updateBlockedDomain(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
        ...
        const result = await database_js_1.default.query('UPDATE blocked_domains SET domain = $1 WHERE id = $2 AND tenant_id = $3 RETURNING *', [domain, id, tenantId]);
        if (result.rowCount && result.rowCount > 0) {
...
async function generateRpzZoneFile(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
        }
        const result = await database_js_1.default.query('SELECT domain FROM blocked_domains WHERE tenant_id = $1 ORDER BY domain ASC', [tenantId]);
...
async function removeAllBlockedDomains(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
        }
        await database_js_1.default.query('DELETE FROM blocked_domains WHERE tenant_id = $1', [tenantId]);
        res.status(204).send();
    }
    catch (error) {
...
async function excludeDomain(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
        ...
        await database_js_1.default.query(`INSERT INTO tenant_domain_exclusions (tenant_id, domain) 
       VALUES ($1, $2) 
       ON CONFLICT (tenant_id, domain) DO NOTHING`, [tenantId, domain]);
...
async function reincludeDomain(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
        ...
        const result = await database_js_1.default.query(`DELETE FROM tenant_domain_exclusions WHERE tenant_id = $1 AND domain = $2`, [tenantId, domain]);
        if (result.rowCount && result.rowCount > 0) {
            res.status(204).send();
        }
        else {
...
async function getExcludedDomains(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId) {
            return res.status(403).json({ error: 'Forbidden: Tenant ID is missing.' });
        }
        const result = await database_js_1.default.query(`SELECT id, domain, excluded_at FROM tenant_domain_exclusions WHERE tenant_id = $1 ORDER BY domain ASC`, [tenantId]);
...
async function getAvailableBlocklists(req, res) {
    try {
        const result = await database_js_1.default.query('SELECT id, name, description, source, created_at FROM dns_blocklists ORDER BY name ASC');
        res.status(200).json(result.rows);
    }
    catch (error) {
...
async function getMySubscriptions(req, res) {
    try {
        ...
        const result = await database_js_1.default.query('SELECT blocklist_id FROM tenant_blocklist_subscriptions WHERE tenant_id = $1', [tenantId]);
...
async function subscribeToBlocklist(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId)
            return res.status(403).json({ error: 'Tenant ID missing' });
        ...
        if (!blocklistId)
            return res.status(400).json({ error: 'blocklistId is required' });
        await database_js_1.default.query('INSERT INTO tenant_blocklist_subscriptions (tenant_id, blocklist_id) VALUES ($1, $2) ON CONFLICT DO NOTHING', [tenantId, blocklistId]);
...
async function unsubscribeFromBlocklist(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId)
            return res.status(403).json({ error: 'Tenant ID missing' });
        const { blocklistId } = req.params;
        await database_js_1.default.query('DELETE FROM tenant_blocklist_subscriptions WHERE tenant_id = $1 AND blocklist_id = $2', [tenantId, blocklistId]);
        res.status(204).send();
    }
    catch (error) {
...
// --- Blocklist Export ---
async function getExportFormats(req, res) {
    res.status(200).json([
...
async function exportBlocklist(req, res) {
    try {
        ...
        if (!tenantId)
            return res.status(400).json({ error: 'Tenant ID required for export' });
        const format = req.query.format || 'hosts';
        // Fetch manual domains
        const manualRes = await database_js_1.default.query('SELECT domain FROM blocked_domains WHERE tenant_id = $1', [tenantId]);
        const manualDomains = manualRes.rows.map(r => r.domain);
        // Fetch subscribed domains
        const subRes = await database_js_1.default.query(`
      SELECT b.domains 
      FROM dns_blocklists b
      JOIN tenant_blocklist_subscriptions s ON b.id = s.blocklist_id
      WHERE s.tenant_id = $1
    `, [tenantId]);
        ...
        const exclusionsRes = await database_js_1.default.query('SELECT domain FROM tenant_domain_exclusions WHERE tenant_id = $1', [tenantId]);
...
const ip_controller_js_1 = require("./ip-controller.js");
async function generateDownloadToken(req, res) {
    try {
        ...
        const countResult = await database_js_1.default.query('SELECT COUNT(*) FROM tenant_download_tokens WHERE tenant_id = $1', [tenantId]);
        ...
        const token = jsonwebtoken_1.default.sign(tokenPayload, JWT_SECRET, { expiresIn: '365d' });
        // Calculate expiry date (1 year from now)
        const expiresAt = new Date();
        expiresAt.setFullYear(expiresAt.getFullYear() + 1);
        // Save token to database (always create new)
        await database_js_1.default.query(`INSERT INTO tenant_download_tokens (tenant_id, token, format, list_type, expires_at)
       VALUES ($1, $2, $3, $4, $5)`, [tenantId, token, format, listType, expiresAt]);
...
async function getDownloadLinkInfo(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId)
            return res.status(403).json({ error: 'Tenant ID missing' });
        const result = await database_js_1.default.query(`SELECT token, format, list_type, expires_at FROM tenant_download_tokens 
...
async function deleteDownloadToken(req, res) {
    try {
        const tenantId = req.user?.tenantId;
        if (!tenantId)
            return res.status(403).json({ error: 'Tenant ID missing' });
        ...
        await database_js_1.default.query('DELETE FROM tenant_download_tokens WHERE tenant_id = $1 AND token = $2', [tenantId, token]);
        res.status(204).send();
    }
    catch (error) {
...
async function downloadBlocklistByToken(req, res) {
    ...
    try {
        ...
        const dbCheck = await database_js_1.default.query('SELECT format, list_type FROM tenant_download_tokens WHERE token = $1 AND tenant_id = $2', [token, decoded.tenantId]);
        ...
        if (listType === 'ip') {
            ...
            return (0, ip_controller_js_1.exportBlockedIps)(req, res);
        }
        else {
