...
const mailersend_js_1 = require("../config/mailersend.js");
...
async function sendRecoveryCode(req, res) {
    ...
    try {
        const userResult = await database_js_1.default.query('SELECT id, name, email FROM users WHERE email = $1', [email]);
        ...
        const emailSent = await (0, mailersend_js_1.sendEmail)({
            to: email,
...
async function resetPassword(req, res) {
    ...
    try {
        ...
        const result = await database_js_1.default.query('UPDATE users SET password_hash = $1 WHERE email = $2 RETURNING id, email', [hashedPassword, email]);
