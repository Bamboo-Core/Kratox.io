...
async function seedDatabase() {
    ...
    `);
        ...
        const tenant1Res = await client.query('INSERT INTO public.tenants (name) VALUES ($1) ON CONFLICT (name) DO NOTHING RETURNING id;', [tenant1Name]);
        ...
        await client.query(`INSERT INTO public.users (tenant_id, name, email, password_hash, role)
       VALUES ($1, $2, $3, $4, 'admin')
       ON CONFLICT (email)
       DO UPDATE SET name = EXCLUDED.name,
                     password_hash = EXCLUDED.password_hash,
                     role = EXCLUDED.role,
                     tenant_id = EXCLUDED.tenant_id;`, [tenant1Id, 'Admin User', adminEmail, adminHashedPassword]);
        console.log(`- User "${adminEmail}" (admin) created or updated. Password is "${adminPassword}"`);
        // TENANT 2: ACME Inc.
        const tenant2Name = 'ACME Inc.';
        const tenant2Res = await client.query('INSERT INTO public.tenants (name) VALUES ($1) ON CONFLICT (name) DO NOTHING RETURNING id;', [tenant2Name]);
        ...
        const acmeHashedPassword = await bcryptjs_1.default.hash(acmePassword, 10);
        const acmeEmail = 'user@acme.inc';
        await client.query(`INSERT INTO public.users (tenant_id, name, email, password_hash, role, zabbix_hostgroup_ids)
       ...
       DO UPDATE SET name = EXCLUDED.name,
                     ...
                     zabbix_hostgroup_ids = EXCLUDED.zabbix_hostgroup_ids;`, [tenant2Id, 'ACME User', acmeEmail, acmeHashedPassword, '{4}']);
        ...
        const tenant3Res = await client.query('INSERT INTO public.tenants (name) VALUES ($1) ON CONFLICT (name) DO NOTHING RETURNING id;', [tenant3Name]);
        ...
        const fibraHashedPassword = await bcryptjs_1.default.hash(fibraPassword, 10);
        // User 1: Ana Silva
        const emailAna = 'ana.silva@fibraveloz.com';
        await client.query(`INSERT INTO public.users (tenant_id, name, email, password_hash, role, zabbix_hostgroup_ids, phone_number)
       ...
       DO UPDATE SET name = EXCLUDED.name,
                     ...
                     phone_number = EXCLUDED.phone_number;`, [tenant3Id, 'Ana Silva', emailAna, fibraHashedPassword, '{15}', '5553991673803']);
       ...
       DO UPDATE SET name = EXCLUDED.name,
                     ...
                     tenant_id = EXCLUDED.tenant_id,
                     zabbix_hostgroup_ids = EXCLUDED.zabbix_hostgroup_ids,
                     phone_number = EXCLUDED.phone_number;`, [tenant3Id, 'Roberto Santos', emailRoberto, fibraHashedPassword, '{15}', '5553981259970']);
       ...
       DO UPDATE SET name = EXCLUDED.name,
                     ...
                     tenant_id = EXCLUDED.tenant_id,
                     zabbix_hostgroup_ids = EXCLUDED.zabbix_hostgroup_ids,
                     phone_number = EXCLUDED.phone_number;`, [tenant3Id, 'Julia Lima', emailJulia, fibraHashedPassword, '{15}', '5553981053550']);
        ...
        const acmeRuleRes = await client.query(`INSERT INTO public.automation_rules (tenant_id, name, trigger_type, trigger_conditions, action_type, action_params)
       VALUES ($1, $2, 'zabbix_alert', '{"alert_name_contains": "malware"}', 'dns_block_domain_from_alert', '{}')
       RETURNING id;`, [tenant2Id, 'Regra de Bloqueio de Malware']);
        ...
        const fibraRuleRes = await client.query(`INSERT INTO public.automation_rules (tenant_id, name, trigger_type, trigger_conditions, action_type, action_params)
       VALUES ($1, $2, 'zabbix_alert', '{"alert_name_contains": "baixa reputação"}', 'dns_block_domain_from_alert', '{}')
       RETURNING id;`, [tenant3Id, 'Bloquear Sites de Baixa Reputação']);
        ...
        await client.query(`INSERT INTO public.automation_criteria (name, label, description, value_type)
       VALUES ('alert_name_contains', 'Alert Name Contains', 'Triggers when the Zabbix alert name includes the specified text.', 'text');`);
        ...
        await client.query(`INSERT INTO public.automation_actions (name, label, description)
       VALUES ('dns_block_domain_from_alert', 'Block Domain from Alert', 'Uses AI to extract a domain from the alert text and adds it to the DNS blocklist.');`);
        ...
        // Log for ACME Inc.
        await client.query(`INSERT INTO public.automation_logs (rule_id, rule_name, tenant_id, trigger_event, action_type, action_details, status, message, executed_at)
