...
exports.handleAutomationNotification = handleAutomationNotification;
exports.processZabbixEvent = processZabbixEvent;
exports.findTenantIdFromHost = findTenantIdFromHost;
...
const netmiko_service_js_1 = require("./netmiko-service.js");
const zabbix_service_js_1 = require("./zabbix-service.js");
const crypto_js_1 = require("../utils/crypto.js");
const whatsapp_service_js_1 = require("./whatsapp-service.js");
async function handleAutomationNotification({ ruleName, tenantId: incomingTenantId, // Renomeamos para evitar confusão
triggerEvent, status, message, }) {
    ...
    if (isManualTest) {
        ...
        try {
            ...
            const hosts = await (0, zabbix_service_js_1.getZabbixHosts)(tenantId, undefined, [hostId], true);
            const host = hosts?.[0];
            if (!host) {
    ...
    try {
        ...
        const userQuery = await database_js_1.default.query(`SELECT name, phone_number FROM users 
         WHERE tenant_id = $1
        ...
        for (const user of userQuery.rows) {
            ...
            await (0, whatsapp_service_js_1.sendWhatsappMessage)(user.phone_number, notificationMessage);
        }
    }
    catch (error) {
...
async function processZabbixEvent(payload) {
    ...
    const event = await (0, zabbix_service_js_1.getZabbixEventById)(payload.eventid);
    if (!event) {
    ...
    if ((0, feature_flag_service_js_1.getFeatureFlag)('scriptable_automation_templates', tenantId)) {
...
const MatchTemplateSchema = zod_1.z.object({
    best_match_template_id: zod_1.z.string().describe("The ID of the template that best matches the alert. If no template is a good match, return 'none'."),
    reasoning: zod_1.z.string().describe("A brief explanation for why this template was chosen.")
});
const matchAlertToTemplate = genkit_js_1.ai.definePrompt({
    name: "matchAlertToTemplate",
    input: { schema: zod_1.z.object({ alert_name: zod_1.z.string(), templates: zod_1.z.array(zod_1.z.object({ id: zod_1.z.string(), trigger_description: zod_1.z.string() })) }) },
...
async function processWithTemplates(tenantId, payload) {
    const templateQuery = await database_js_1.default.query(`SELECT t.id, t.name, t.trigger_description, t.action_script 
         FROM automation_templates t
         JOIN tenant_template_subscriptions s ON t.id = s.template_id
         WHERE s.tenant_id = $1 AND t.is_enabled = true`, [tenantId]);
    const activeTemplates = templateQuery.rows;
...
async function executeTemplateActionAndLog(template, tenantId, payload) {
    let status = 'success';
    let message = '';
    let action_details = {
    ...
    try {
        ...
        const hostsInTenant = await (0, zabbix_service_js_1.getZabbixHosts)(tenantId, undefined, [hostId]);
        ...
        let targetInterface = targetHost.interfaces.find(iface => iface.type === '2');
        if (!targetInterface) {
        ...
        const credsResult = await database_js_1.default.query('SELECT username, encrypted_password, port, device_type FROM device_credentials WHERE host_id = $1 AND tenant_id = $2', [targetHost.hostid, tenantId]);
        const credentials = credsResult.rows[0];
        const commandOutputs = [];
        for (const command of template.action_script.split('\n').filter(Boolean)) {
            const output = await (0, netmiko_service_js_1.executeCommandViaNetmiko)({
                host: hostIp,
                device_type: credentials.device_type,
                command: command,
                username: credentials.username,
                password: (0, crypto_js_1.decrypt)(credentials.encrypted_password),
    ...
    try {
        ...
        `;
        await database_js_1.default.query(logQuery, [null, template.name, tenantId, payload, 'run_script_from_template', action_details, status, message]);
...
async function processWithLegacyRules(tenantId, payload) {
...
async function findTenantIdFromHost(hostId) {
    if (!hostId)
        return null;
    try {
        const credsQuery = await database_js_1.default.query('SELECT tenant_id FROM device_credentials WHERE host_id = $1 LIMIT 1', [hostId]);
        if (credsQuery.rowCount && credsQuery.rowCount > 0) {
            return credsQuery.rows[0].tenant_id;
        }
        const host = (await (0, zabbix_service_js_1.getZabbixHosts)('system-lookup', undefined, [hostId]))[0];
        if (!host || !host.groups || host.groups.length === 0) {
            return null;
        }
        const hostGroupIds = host.groups.map(g => g.groupid);
        const userWithGroupQuery = await database_js_1.default.query(`SELECT tenant_id FROM users WHERE zabbix_hostgroup_ids && $1::text[] LIMIT 1`, [hostGroupIds]);
...
async function getActiveRulesForTenant(tenantId) {
    const result = await database_js_1.default.query('SELECT * FROM automation_rules WHERE tenant_id = $1 AND is_enabled = true', [tenantId]);
    return result.rows;
}
...
async function executeActionAndLog(rule, payload) {
    let status = 'success';
    let message = '';
    let action_details = {};
    try {
    ...
    try {
        const logQuery = `
            ...
            VALUES ($1, $2, $3, $4, $5, $6, $7, $8)
        `;
        await database_js_1.default.query(logQuery, [rule.id, rule.name, rule.tenant_id, payload, rule.action_type, action_details, status, message]);
        await handleAutomationNotification({
            ruleName: rule.name,
...
async function actionBlockDomainFromAlert(tenantId, alertText) {
    console.log(`Executing action: dns_block_domain_from_alert for tenant ${tenantId}`);
    const extractionResult = await (0, extract_domains_flow_js_1.extractDomainsFromText)({ text: alertText });
    ...
    for (const domain of domains) {
        try {
            await database_js_1.default.query('INSERT INTO blocked_domains (domain, tenant_id, source_list_id) VALUES ($1, $2, NULL) ON CONFLICT (domain, tenant_id) DO NOTHING', [domain, tenantId]);
            blocked.push(domain);
        }
        catch (dbError) {
