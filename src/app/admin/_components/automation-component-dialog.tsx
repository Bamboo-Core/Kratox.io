...
export function AutomationComponentDialog({ isOpen, onClose, type, item }: Props) {
  ...
  const onSubmit = (values: AutomationComponentFormData) => {
    ...
    mutation.mutate(params as any, {
      onSuccess: () => {
        toast({ title: 'Success', description: `${type.charAt(0).toUpperCase() + type.slice(1)} ${isEditMode ? 'updated' : 'created'}.` });
        onClose();
      },
      onError: (err: Error) => {
  ...
  const isSubmitting = createCriterion.isPending || updateCriterion.isPending || createAction.isPending || updateAction.isPending;
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
            <FormField control={form.control} name="label" render={({ field }) => (
                <FormItem><FormLabel>Label</FormLabel><FormControl><Input placeholder="User-friendly name" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            <FormField control={form.control} name="name" render={({ field }) => (
                <FormItem><FormLabel>System Name</FormLabel><FormControl><Input placeholder="system_name_without_spaces" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
             <FormField control={form.control} name="description" render={({ field }) => (
                <FormItem><FormLabel>Description</FormLabel><FormControl><Input placeholder="What does this do?" {...field} /></FormControl><FormMessage /></FormItem>
            )} />
            
            {type === 'criterion' && (
                <FormField control={form.control} name="value_type" render={({ field }) => (
                    <FormItem><FormLabel>Value Type</FormLabel>
