...
export function AutomationTemplateDialog({ isOpen, onClose, template }: Props) {
  ...
  const handleSuggestScript = () => {
    ...
    if (!triggerDescription || !deviceVendor) {
      toast({ variant: 'destructive', title: 'Atenção', description: 'Por favor, preencha a descrição do gatilho e o fabricante do dispositivo para obter uma sugestão.' });
      return;
    }

    suggestScriptMutation.mutate({ trigger_description: triggerDescription, device_vendor: deviceVendor }, {
        onSuccess: (data) => {
    ...
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl">
        <DialogHeader>
          ...
          <DialogDescription>Define a trigger and a script-based action to automate a network task.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4 py-2">
            <FormField control={form.control} name="name" render={({ field }) => (
              <FormItem><FormLabel>Template Name</FormLabel><FormControl><Input placeholder="e.g., Diagnose High CPU on Cisco Routers" {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
             <FormField control={form.control} name="description" render={({ field }) => (
              <FormItem><FormLabel>Description</FormLabel><FormControl><Input placeholder="What this automation does" {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <FormField control={form.control} name="trigger_description" render={({ field }) => (
                    <FormItem><FormLabel>Trigger Alert Description</FormLabel><FormControl><Input placeholder="e.g., High CPU utilization on a device" {...field} /></FormControl><FormMessage /></FormItem>
                )}/>
                 <FormField control={form.control} name="device_vendor" render={({ field }) => (
                    <FormItem><FormLabel>Device Vendor</FormLabel>
                        <Select onValueChange={field.onChange} defaultValue={field.value}>
                            <FormControl><SelectTrigger><SelectValue placeholder="Select vendor for script" /></SelectTrigger></FormControl>
                            <SelectContent>
            ...
            <Separator />
             ...
             <h3 className="text-lg font-medium">Action Script</h3>
              <Button type="button" variant="outline" size="sm" onClick={handleSuggestScript} disabled={suggestScriptMutation.isPending}>
                {suggestScriptMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <Sparkles className="mr-2 h-4 w-4" />}
                Suggest Script with AI
            </Button>
            <FormField control={form.control} name="action_script" render={({ field }) => (
                <FormItem><FormLabel>Script</FormLabel><FormControl><Textarea rows={6} placeholder="show version&#10;show interfaces" {...field} /></FormControl><FormMessage /></FormItem>
            )}/>
