...
import { useTenantsQuery, useCreateTenantMutation, useUpdateTenantMutation, tenantFormSchema, type TenantFormData, type Tenant } from '@/hooks/useAdminManagement';
...
export default function TenantsTab() {
  ...
  const onSubmit = (values: TenantFormData) => {
    ...
    mutation.mutate(params as any, {
      onSuccess: () => {
        toast({ title: "Success", description: `Tenant ${isEditMode ? 'updated' : 'created'} successfully.` });
        handleCloseDialog();
      },
      onError: (err: Error) => {
        toast({
      ...
      <Dialog open={isDialogOpen} onOpenChange={handleCloseDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {isEditMode ? `Update details for ${selectedTenant?.name}.` : 'Add a new tenant to the platform.'}
            </DialogDescription>
          </DialogHeader>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              <FormField control={form.control} name="name" render={({ field }) => (
                  <FormItem><FormLabel>Tenant Name</FormLabel><FormControl><Input placeholder="e.g., ACME Corporation" {...field} /></FormControl><FormMessage /></FormItem>
              )}/>
               <FormField control={form.control} name="probe_api_url" render={({ field }) => (
                  <FormItem><FormLabel>Probe API URL (Optional)</FormLabel><FormControl><Input placeholder="http://customer-probe.internal/api" {...field} /></FormControl><FormMessage /></FormItem>
              )}/>
              <DialogFooter>
            ...
            {tenants.length > 0 ? (
              tenants.map((tenant) => (
                <TableRow key={tenant.id}>
                  ...
                  <TableCell className="font-mono text-xs">{tenant.probe_api_url || 'Not set'}</TableCell>
