...
import {
  useAutomationTemplatesForClient,
  ...
  if (isLoading) {
    return (
      <div className="text-center p-4">
        <Loader2 className="h-5 w-5 animate-spin inline-block" /> Carregando templates...
      </div>
    );
  }
  if (isError) {
        ...
        <TableBody>
          {data && data.templates.length > 0 ? (
            data.templates.map((template) => {
              ...
              const isMutating =
                (subscribeMutation.isPending && subscribeMutation.variables === template.id) ||
              ...
              return (
                <TableRow key={template.id}>
                  ...
                  <TableCell>
                    <Badge variant="outline">{template.device_vendor}</Badge>
                  </TableCell>
                </TableRow>
              );
            })
            ...
            <TableRow>
              <TableCell colSpan={4} className="text-center h-24">
                Nenhum template de automação disponível.
              </TableCell>
            </TableRow>
          )}
        </TableBody>
