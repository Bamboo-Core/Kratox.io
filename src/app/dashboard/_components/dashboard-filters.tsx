...
export default function DashboardFilters({
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap justify-end w-full md:w-auto">
      {isAdmin && (
        <Select
          onValueChange={setHostGroupFilter}
        ...
        >
          ...
          <SelectContent>
            <SelectItem className="focus:bg-orange-500 focus:text-white" value="all">
              Todos os Grupos
            </SelectItem>
            {hostGroups.map((hg) => (
              <SelectItem
      ...
      <Select onValueChange={setSeverityFilter} value={severityFilter}>
        ...
        <SelectContent>
          ...
          {Object.entries(severityMap)
            .reverse()
            .map(([key, { text }]) => (
              <SelectItem className="focus:bg-orange-500 focus:text-white" key={key} value={key}>
                {text}
              </SelectItem>
            ))}
        </SelectContent>
      ...
      <Select onValueChange={handleDatePresetChange} value={datePreset}>
        ...
        <SelectContent>
          <SelectItem className="focus:bg-orange-500 focus:text-white" value="today">
            Hoje
          </SelectItem>
          <SelectItem className="focus:bg-orange-500 focus:text-white" value="yesterday">
            Ontem
          </SelectItem>
          <SelectItem className="focus:bg-orange-500 focus:text-white" value="7days">
            Últimos 7 dias
          </SelectItem>
          <SelectItem className="focus:bg-orange-500 focus:text-white" value="30days">
            Últimos 30 dias
          </SelectItem>
          <SelectItem className="focus:bg-orange-500 focus:text-white" value="custom">
      ...
      <Popover>
        <PopoverTrigger asChild>
          <Button
            variant={'outline'}
            className={cn(
              'w-full sm:w-[240px] justify-start text-left font-normal hover:bg-orange-500 hover:text-white',
