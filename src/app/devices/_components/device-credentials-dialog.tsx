...
import {
  useSaveDeviceCredentialsMutation,
  type DeviceCredentialsFormData,
...
export function DeviceCredentialsDialog({
  ...
  const onSubmit = (values: DeviceCredentialsFormData) => {
    ...
    saveCredentialsMutation.mutate(
      { hostId: host.hostid, ...values },
      {
        onSuccess: () => {
          toast({
            title: 'Success',
            description: `Credentials for ${host.name} saved successfully.`,
          });
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      <DialogContent className="sm:max-w-[425px]">
        <DialogHeader>
          ...
          <DialogDescription>
            Enter the SSH credentials for <span className="font-bold font-mono">{host?.name}</span>.
