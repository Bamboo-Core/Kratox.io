  ...
  return (
    <div className="flex flex-col h-full">
      ...
      <main className="flex-1 p-4 md:p-6 space-y-6 overflow-y-auto">
        ...
        <Card className="shadow-lg">
          <CardHeader><div className="flex justify-between items-start"><div><CardTitle className="flex items-center gap-2"><ShieldAlert className="h-6 w-6 text-primary" />Domínios Bloqueados Atualmente</CardTitle><CardDescription>Lista de domínios sendo bloqueados para seu tenant.</CardDescription></div><Button onClick={handleGenerateRpz} disabled={blockedDomains.length === 0 || generateRpzFileMutation.isPending}>{generateRpzFileMutation.isPending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Download className="mr-2 h-4 w-4" />}Gerar Arquivo RPZ</Button></div></CardHeader>
          <CardContent>
