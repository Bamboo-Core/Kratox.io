...
export default function DnsBlockingPage() {
  ...
  const handleBlockAllSuggested = () => {
    ...
    Promise.all(
      ...
      toast({
        title: 'Success',
        description: `${domainsToBlock.length} domains have been added to the blocklist.`,
      });
      setSuggestedDomains([]);
    });
  };
  ...
  const handleTextAnalysisSuccess = (data: { domains: string[] }) => {
    ...
    if (newDomains.length === 0) {
      ...
      toast({
        ...
        description: `${newDomains.length} new domain(s) found.`,
      });
    }
  };
      ...
      <main className="flex-1 p-4 md:p-6 space-y-6 overflow-y-auto">
        ...
        <Card className="shadow-lg">
          <CardHeader>
            ...
            <CardDescription>
              Inscreva-se em listas de bloqueio gerenciadas para proteção automática contra ameaças
              conhecidas.
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            {availableBlocklistsQuery.isLoading && (
              <div className="text-center">
                <Loader2 className="h-6 w-6 animate-spin inline-block" /> Carregando feeds...
              </div>
            )}
            {availableBlocklists.map((list) => {
              const isSubscribed = mySubscriptions.includes(list.id);
              const isMutating =
                (subscribeMutation.isPending && subscribeMutation.variables === list.id) ||
                ...
                >
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      {list.description} (Fonte: {list.source})
                    </p>
        ...
        {suggestedDomains.length > 0 && (
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Estes domínios foram encontrados na sua análise e ainda não estão na lista de
                bloqueio.
              </CardDescription>
            </CardHeader>
            <CardContent>
                ...
                {suggestedDomains.map((domain) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={
                        addDomainMutation.isPending && addDomainMutation.variables === domain
                      }
        ...
        <Card className="shadow-lg">
          <CardHeader>
            <div className="flex justify-between items-start">
              ...
              <Button
                onClick={handleGenerateRpz}
                className="bg-orange-500 hover:bg-orange-600 cursor-pointer"
                disabled={blockedDomains.length === 0 || generateRpzFileMutation.isPending}
              >
                {generateRpzFileMutation.isPending ? (
                  <Loader2 className="mr-2 h-4 w-4 animate-spin" />
                ) : (
                    ...
                    {paginatedDomains.map((item) => (
                      <TableRow
                        ...
                        className={
                          ...
                          removeDomainMutation.variables === item.id
                            ? 'opacity-50'
                            : ''
                        }
                      >
                        ...
                        <TableCell>
                          <Badge variant={item.source_list_name ? 'secondary' : 'outline'}>
                            {item.source_list_name || 'Manual'}
                          </Badge>
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell className="text-right">
                          <Button
                            ...
                            className="hover:text-white hover:bg-orange-500 cursor-pointer"
                            disabled={removeDomainMutation.isPending || !!item.source_list_id}
                            title={
                              item.source_list_id
                                ? 'Cancele a inscrição do feed para remover'
