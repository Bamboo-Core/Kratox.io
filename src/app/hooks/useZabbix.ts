...
const fetchZabbixHostById = async (
  token: string | null,
  hostId: string
): Promise<ZabbixHost | null> => {
  if (!token) throw new Error('Authentication token is missing.');
...
const fetchZabbixItemsForHost = async (
  token: string | null,
  hostId: string
): Promise<ZabbixItem[]> => {
  if (!token) throw new Error('Authentication token is missing.');
...
const fetchZabbixItemHistory = async (
  token: string | null,
  itemId: string,
  historyType: '0' | '3',
  dateRange?: DateRange
  ...
  const response = await fetch(
    `${API_BASE_URL}/api/zabbix/items/${itemId}/history?${params.toString()}`,
    {
      headers: getAuthHeader(token),
    }
  );
...
const suggestCommands = async (
  payload: SuggestCommandsPayload,
  token: string | null
): Promise<SuggestCommandsResponse> => {
  if (!token) throw new Error('Authentication token is missing.');
...
const fetchZabbixItemsByEvent = async (
  token: string | null,
  eventId: string
): Promise<ZabbixItem[]> => {
  if (!token) throw new Error('Authentication token is missing.');
...
const diagnoseNetwork = async (
  payload: DiagnoseNetworkPayload,
  token: string | null
  ...
  if (!response.ok) {
    const errorData = await response
      .json()
      .catch(() => ({ message: 'Failed to diagnose network' }));
    throw new Error(
...
export const useZabbixItemHistoryQuery = (
  itemId?: string,
  historyType?: '0' | '3',
  dateRange?: DateRange
) => {
